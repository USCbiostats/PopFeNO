...
* The intercepts and slopes of the linear regression moedls: NOparam ~ X of the simulated data:
```{r}
...
abline(v=(exp(estParam[2,2])-1),col="gray")
# add 95% CI
...
abline(v=(exp(estParam[2,3])-1),col="gray")
# add 95% CI
...
## 4: Comparison by table:

* Betas:

```{r}
all2<-rbind(allResults[1:3,3],allResults[4:6,3],allResults[7:9,3],allResults[10:12,3],allResults[13:15,3],allResults[16:18,3])
colnames(all2)<-c("CaNO","logCaw","logDaw")
rownames(all2)<-sixMethods
kable(round(all2,2))
getwd()
```
