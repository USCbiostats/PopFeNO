...
##-----------unbalanced data --------------##
TSHB_JagsModel_UB<-function()  {# subject:i, flows: list of flow for each subjects
  for( i in 1 : Ndat) {
    for( j in offset[i]:(offset[i+1]-1)) {
      logeno[j] ~ dnorm(logmu[j],tau_c)#logmu new
...
#####----------------TSHB ub function ----------------------#
TSHB_iter_UB<-function(beta0_prior=c(2,4,3),
  ...
  # create offset of flows
  offset<-c(1,cumsum(table(dat$id))+1)
  
  # data needed for JAGS model
  data_TSHB_UB <- list(Ndat=length(table(dat$id)),logeno=dat$logeno,offset=offset,flow=dat$flow,
  ...
  time<-proc.time()[3]
  # Jags function
  fit_TSHB<- jags(data=data_TSHB_UB, parameters.to.save=param_TSHB, inits=inits_TSHB,
                  model.file=TSHB_JagsModel_UB, n.chains=N.chain, n.iter=N.iterT, 
